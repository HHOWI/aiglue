...
- **`aiglue mcp serve --transport http`** runs the MCP server over StreamableHTTP so it can be hosted centrally. Stateless mode — each request creates its own server + transport pair so the request's `Authorization: Bearer …` header flows into `Executor.execute()` as the upstream `authToken`. `AIGLUE_AUTH_TOKEN` env var becomes a fallback for single-tenant setups. Default bind is `127.0.0.1`; expose externally only behind a TLS-terminating reverse proxy with auth.
